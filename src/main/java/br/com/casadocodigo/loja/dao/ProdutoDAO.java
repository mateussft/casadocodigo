...
public class ProdutoDAO {
	...
	public List<Produto> relatar() {
		List<Produto> produtos = manager.createQuery("select distinct(p) from Produto p join fetch p.precos", Produto.class)
				.getResultList();
		return produtos;
	}
	
	public List listarPorData(Calendar data) {		
		return manager.createQuery("select p from Produto p "		
	+ "where p.dataLancamento > :pData", Produto.class)				
